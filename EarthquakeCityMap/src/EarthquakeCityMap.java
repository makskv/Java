...
public class EarthquakeCityMap extends PApplet {
	...
	public void setup() {		
	    ...
	    /* Implementation of sorting earthquakes by magnitude 
	     * and printing x largest earthquakes, 
	     * where x is number given in parameter
	     */
	...
	public void draw() {
	   ...
	   private void sortAndPrint(int numToPrint) {
		   ...
		   Object[] quake = quakeMarkers.toArray();
		   Arrays.sort(quake);
		   if (numToPrint > 0 && numToPrint < quake.length) {
			   
			   for (int i = quake.length - 1; i>= quake.length - numToPrint; i--) {
				   
				   String title = ((EarthquakeMarker) quake[i]).getTitle();
				   if (quake[i] instanceof LandQuakeMarker) {
					   String country = ((LandQuakeMarker) quake[i]).getCountry();
